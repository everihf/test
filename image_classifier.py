...
class ImageClassifier():
    ...
    def _init_dataloader(self, data_name):
        ...
        self.valid_loader = torch.utils.data.DataLoader(
            valid_dataset, batch_size=512, shuffle=False, num_workers=8, pin_memory=True)
        self.test_loader = torch.utils.data.DataLoader(
            test_dataset, batch_size=512, shuffle=False, num_workers=8, pin_memory=True)
    ...
    def _train(self):
        ...
        for epoch in range(self.epochs):
            ...
            self.logger.info(f"[{epoch+1:3d}] loader dataset size = {len(loader.dataset)}")#################
